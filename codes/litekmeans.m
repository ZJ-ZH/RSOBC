...
%将矩阵X（N*P）划分为K个簇 N为点个数，P为点的特征个数  center：返回K个聚类质心位于??矩阵K*P中心的位置 label:返回包含每个点对应的聚类索引，即N×1的矢量标签。
% bCon： 返回bool值bCon 表示迭代是否收敛   sumD：在1×K向量中的，每个聚类其中点到该质心距离的群内总和 sumD。
